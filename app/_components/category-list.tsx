...
export const CategoryList = ({ categoryList }: CategoryListProps) => {
  return (
    <div className='mx-4 md:mx-22 lg:mx-52 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 my-8'>

      {/* fetching the categories from the backend and passing it to the ui */}
      {categoryList.length > 0 ? categoryList.map((category, index ) => (
        <Link href={'/search/'+category.name} key={index} className="flex flex-col items-center justify-center gap-2 bg-[#F1F1F1] p-5 rounded-lg w-32 hover:scale-105 hover:border-primary hover:border transition-all ease-in-out shadow cursor-pointer">
          <Image
